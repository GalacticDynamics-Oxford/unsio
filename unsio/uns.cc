...
//  ---------------------------------------------------------------------------
// constructor
CunsOut::CunsOut(const char* _name, const char* _type, const bool _verb )
{
  simname  = tools::Ctools::fixFortran(_name,false);
  simtype  = tools::Ctools::fixFortran(_type,false);
  verbose = _verb;
  snapshot= NULL;
  if (verbose) {
